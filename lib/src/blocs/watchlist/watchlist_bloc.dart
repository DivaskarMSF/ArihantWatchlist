...
class WatchlistBloc extends BaseBloc<WatchlistEvent, WatchlistState> {
  ...
  @override
  Future<void> eventHandlerMethod(WatchlistEvent event, Emitter<WatchlistState> emit) async {
  ...
  Future<void> _handleWatchlistGetGroupsEvent(WatchlistGetGroupsEvent event, Emitter<WatchlistState> emit) async {

      emit(WatchlistProgressState());
    try {
      final BaseRequest request = BaseRequest();
      final WatchlistGroupModel watchlistGroupModel = await WatchlistRepository().getWatchlistGroupsRequest(request);
  ...
  Future<void> _handleWatchlistGetSymbolsEvent(WatchlistGetSymbolsEvent event, Emitter<WatchlistState> emit) async {
    emit(WatchlistProgressState());

    try {
      final BaseRequest request = BaseRequest(data: event.group.toJson());
      final symbolCacheModel = await CacheRepository.watchlistCache.get(event.group.wName ?? "");
      WatchlistSymbolsModel? watchlistSymsModel;
      if (symbolCacheModel is WatchlistSymbolsModel) {
        watchlistSymsModel = WatchlistSymbolsModel(symbolCacheModel.getSymbols().map((symbol) => Symbols.copyModel(symbol)).toList(),);
      ...
      if (event.fetchApi || (watchlistSymsModel?.symbols.isEmpty ?? true)) {
        watchlistSymsModel = await WatchlistRepository().getWatchlistSymbolsRequest(request, wId: event.group.wId!);
      }
...
//for delete symbol only sym block needs to be sent.
  ...
  Future<void> responseCallback(ResponseData streamData, Emitter<WatchlistState> emit) async {

    if (watchlistDoneState.watchlistSymbolsModel != null) {
      final List<Symbols> symbols = watchlistDoneState.watchlistSymbolsModel!.symbols;
      ...
      if (index != -1) {
        ...
        _handleWatchlistFilterSortSymbolEvent(WatchlistFilterSortSymbolEvent(watchlistDoneState.selectedWatchlist, 
            watchlistDoneState.watchlistGroupModel!.groups!.firstWhere((element) => element.wName == watchlistDoneState.selectedWatchlist).selectedSortBy,
                watchlistDoneState.watchlistGroupModel!.groups!
                    .firstWhere((element) => element.wName == watchlistDoneState.selectedWatchlist).selectedFilter), emit);
